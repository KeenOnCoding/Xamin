...
if (typeof window !== "undefined") {
  window.$ = $;
  window.jQuery = $;
  window.tpj = jQuery.noConflict();
  import("../public/assets/revslider/js/revolution.tools.min");
  import("../public/assets/revslider/js/rs6.min");
  require("../public/assets/js/appear");
  require("../public/assets/js/isotope.pkgd.min");
  require("../public/assets/js/Chart.min");
  require("../public/assets/js/Chart.bundle.min");
  require("popper.js");
  require("bootstrap");
  WOW = require("wowjs");
...
const magnific = () => {
  $(".popup-gallery").magnificPopup({
    ...
    image: {
      tError:
        '<Link href="%url%">The image #%curr%</Link> could not be loaded.',
    },
  });
...
export const initRevSlider1 = () => {
  if (!tpj("#rev_slider_0_1").hasClass("revslider-initialised")) {
    ...
    setTimeout(() => {
      console.log("test 1");
      if (typeof tpj("#rev_slider_0_1")?.revolution == "undefined") {
        setTimeout(() => {
          initRevSlider1();
        }, 100);
      } else {
...
const chartInit = () => {
  if ($("#data-analytics-pie-chart").length) {
    ...
    var ctx = $("#data-analytics-pie-chart");
...
const chart = () => {
  if ($("#managed-data-analytics").length) {
    ...
    var ctx = $("#managed-data-analytics");
...
const artificialIntelligenceChart = () => {
  if ($("#artificial-intelligence").length) {
    ...
    var ctx = window.jQuery("#artificial-intelligence");
...
const dataVisualizationChart = () => {
  if ($("#data-visualization").length) {
    ...
    var myPieChart = new Chart(ctx, {
      type: "bar",
      data: data,
      options: { scaleBeginAtZero: true, legend: { display: false } },
    });
    document.getElementById("js-legend").innerHTML =
      myPieChart.generateLegend();
  }
...
const pillTab = () => {
  var nav = window.jQuery("#pills-tab");
  if (nav.length) {
    window.jQuery(window).on("scroll", function (e) {
      var contentNav = nav.offset().top - window.outerHeight;
      if (window.jQuery(window).scrollTop() >= contentNav) {
        event.preventDefault();
        window.jQuery("#pills-tab li a").removeClass("active");
        window.jQuery("#pills-tab li a[aria-selected=true]").addClass("active");
      }
    });
  }
};
