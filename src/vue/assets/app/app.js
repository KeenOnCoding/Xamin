...
if (typeof window !== "undefined") {
  ...
  const revolution = () =>
    import("../../static/revslider/js/revolution.tools.min.js");
...
export const themes = {
  ...
  async wowInit() {
    ...
    if (elementExist) {
      // eslint-disable-next-line no-undef
      const varWow = await WOW.finally();
      new varWow.default({
        boxClass: "wow",
  ...
  async skrollr() {
    if ($skrollr) {
      try {
        const skrollrModule = await $skrollr;
        const skrollrInstance = skrollrModule.default;
        ...
        console.error("Failed to load skrollr:", error);
      }
    }
  },
  ...
  chartJs() {
    ...
    if (elementExist) {
      ...
      if (this.checkElement("class", "big-data-services-chart")) {
        const data = {
          datasets: [
            { data: [60, 40], backgroundColor: ["#fe6c61", "#5471d2"] },
          ],
          labels: ["One", "Two"],
        };
        const ctx = $("#canvas");
  ...
  accordion() {
    if (this.checkElement("class", "iq-faq")) {
      ...
      jQuery(".iq-faq .iq-block:first")
        .addClass("iq-active")
        .children()
        .slideDown("slow");
      jQuery(".iq-faq .iq-block").on("click", function () {
        if (jQuery(this).children("div").is(":hidden")) {
          jQuery(".iq-faq .iq-block")
